...
RUSTFLAGS="-C target-feature=+neon -C opt-level=3 -C strip=symbols" cargo +nightly build -Z build-std=std,panic_abort --target aarch64-linux-android --release --manifest-path Cargo.toml
